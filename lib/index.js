...
var ROLE_OPER_GROUP = sprintf(GROUP_FMT, 'role-operators');
...
// AccessKeys format:
var USER_ACCESS_KEY_FMT = 'accesskeyid=%s, ' + USER_FMT;
var SUBUSER_ACCESS_KEY_FMT = 'accesskeyid=%s, ' + SUBUSER_FMT;

// --- Internal Functions

// Several function have a 'user' which might be an object with any of:
//
// .uuid, .login or .dn
//
// or might be a string representation of one of these. This method takes
// one of these messy 'user' objects and returns a string.
function userToString(user) {
    return (user.uuid || user.login || user.dn ||
...
function UFDS(opts) {
    ...
    assert.optionalObject(opts.tracer, 'options.tracer');
...
UFDS.prototype.authenticate = function authenticate(login, pwd, account, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceAuthenticate(span, tracedCb) {
        var entry;
        ...
        function _compare(user) {
            self.compare(user.dn, 'userpassword', pwd, function (err, ok) {
                if (err) {
                    ...
                    span.addTags({validCredentials: false});
                    cb(new InvalidCredentialsError('The credentials ' +
                                                   'provided are invalid'));
                } else {
                    span.addTags({validCredentials: true});
                    if (self.cache) {
        ...
        if (self.cache && (entry = self.cache.get(cacheKey))) {
            span.addTags({cacheHit: true, validCredentials: true});
            cb(null, entry);
        } else if (typeof (login) === 'object') {
            _compare(login);
        } else {
            self.getUser(login, account, function (err, user) {
                if (err) {
                    cb(err);
                } else {
                    _compare(user);
...
UFDS.prototype.addUser = function addUser(user, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceAddUser(span, tracedCb) {
        cb = tracedCb;
        ...
        self.add(dn, user, function (add_err) {
            ...
            self.getUserEx(searchOpts, function (err, obj) {
                if (err) {
                    cb(err);
                    return;
                }
...
UFDS.prototype._getUser = function _getUser(filter, msg, account, cb, noCache) {
    ...
    self.search(base, opts, function (err, entries) {
        if (err) {
            cb(err);
            return;
        }
...
UFDS.prototype.getUserEx = function getUserEx(opts, cb) {
    ...
    self._traceFunction(cb, function _traceGetUserEx(span, tracedCb) {
        cb = tracedCb;

        switch (opts.searchType) {
        case 'uuid':
            /*
             * Note that the UUID filter is the same whether or not we are
             * looking for a top-level account or a sub-user.  In either case,
             * the base DN of the search is set appropriately by _getUser().
             */
        ...
        self._getUser(filter, msg, opts.account, cb, opts.noCache);
    });
};
...
UFDS.prototype.getUser = function getUser(login, account, cb, noCache) {
    ...
    self._traceFunction(cb, function _traceGetUser(span, tracedCb) {
        cb = tracedCb;
        ...
        self._getUser(filter, msg, account, cb, noCache);
    });
};
...
function getUserByEmail(email, account, cb, noCache) {
    ...
    self._traceFunction(cb, function _traceGetUserByEmail(span, tracedCb) {
        cb = tracedCb;
...
UFDS.prototype.updateUser = function updateUser(user, changes, account, cb) {
    ...
    self._traceFunction(cb, function _traceUpdateUser(span, tracedCb) {
        cb = tracedCb;
...
UFDS.prototype.deleteUser = function deleteUser(user, account, cb) {
    ...
    self._traceFunction(cb, function _traceDeleteUser(span, tracedCb) {
        cb = tracedCb;
        ...
        if (typeof (user) === 'object') {
            ...
            self.getUser(user, account, _delete);
        }
    });
};
...
UFDS.prototype.addKey = function addKey(user, key, account, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceAddKey(span, tracedCb) {
        cb = tracedCb;
        ...
        function _addKey(init_err, user) {
            ...
            var fingerprint;
            try {
                var pkey = sshpk.parseKey(key.openssh, 'ssh');
                fingerprint = pkey.fingerprint('md5').toString('hex');
            } catch (e) {
        ...
        if (typeof (user) === 'object') {
            ...
            self.getUser(user, account, _addKey);
        }
    });
};
...
UFDS.prototype.getKey = function getKey(user, fp, account, cb, noCache) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceGetKey(span, tracedCb) {
        cb = tracedCb;
        ...
        span.addTags({
            ...
            fingerprint: fp,
            noCache: Boolean(noCache),
            user: user.uuid || user.login || user.dn || JSON.stringify(user)
        });
        ...
        function _keys(err, user) {
            if (err) {
                ...
                var filter = util.format(
                    '(&(objectclass=sdckey)(|(name=%s)(fingerprint=%s)))',
                    fp, fp);
                var opts = {
                    scope: 'one',
                    filter: filter
                ...
                self.search(user.dn, opts, function (err, keys) {
                    if (err) {
                        ...
                        if (keys.length) {
                            ...
                            cb(new ResourceNotFoundError(fp +
                                ' does not exist'));
                        }
                    }
                }, noCache);
...
UFDS.prototype.listKeys = function listKeys(user, account, cb, noCache) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceListKeys(span, tracedCb) {
        cb = tracedCb;
...
UFDS.prototype.deleteKey = function deleteKey(user, key, account, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceDeleteKey(span, tracedCb) {
        cb = tracedCb;
        ...
        function _delKey(user, key) {
            if (!ldap.parseDN(user.dn).parentOf(key.dn)) {
                cb(new NotAuthorizedError(key.dn + ' not a child of ' +
                    user.dn));
            } else {
                self.del(key.dn, cb);
            }
        ...
        if (typeof (user) === 'object') {
            ...
            self.getUser(user, account, function (err, user) {
                if (err) {
                    cb(err);
                } else {
                    _getKey(user);
...
UFDS.prototype.listLimits = function listLimits(user, account, cb, noCache) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceListLimits(span, tracedCb) {
        cb = tracedCb;
        ...
        span.addTags({
            account: account,
            noCache: Boolean(noCache),
            searchOpts: JSON.stringify(opts),
            user: userToString(user)
        });
...
UFDS.prototype.getLimit = function getLimit(user, dc, account, cb, noCache) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceGetLimit(span, tracedCb) {
        cb = tracedCb;
        ...
        function _limits(init_err, user) {
            ...
            self.listLimits(user, account, function (err, limits) {
                ...
                if (!limits.some(function (l) {
                    ...
                    cb(new ResourceNotFoundError(sprintf(
                        'No limit found for %s/%s', user.login, dc)));
                } else {
                    cb(null, limit);
                }
            }, noCache);
        ...
        if (typeof (user) === 'object') {
            ...
            self.getUser(user, account, _limits);
        }
    });
};
...
UFDS.prototype.addLimit = function addLimit(user, limit, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceAddLimit(span, tracedCb) {
        cb = tracedCb;
...
UFDS.prototype.updateLimit = function updateLimit(user, limit, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceUpdateLimit(span, tracedCb) {
        cb = tracedCb;
...
UFDS.prototype.deleteLimit = function deleteLimit(user, limit, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceDeleteLimit(span, tracedCb) {
        cb = tracedCb;
...
UFDS.prototype.listForeigndc = function listForeigndc(user, authdev, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceListForeigndc(span, tracedCb) {
        cb = tracedCb;
...
function listDcLocalConfig(uuid, account, cb, noCache) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceListDcLocalConfig(span, tracedCb) {
        cb = tracedCb;
        ...
        self.search(base, opts, function (err, data) {
            if (err) {
                return cb(err);
            }
            ...
            default:
                var _err = new Error(util.format('Multiple results for %s ' +
                    ...
                    account));
                self.log.error({ err: _err, results: data },
...
function getDcLocalConfig(uuid, account, datacenter, cb, noCache) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceGetDcLocalConfig(span, tracedCb) {
        var opts = {
            scope: 'base'
        };
        ...
        self.search(dn, opts, function (err, data) {
            var cfg = {};
            if (err) {
                return cb(err);
            }
...
function addDcLocalConfig(uuid, account, datacenter, config, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceAddDcLocalConfig(span, tracedCb) {
        cb = tracedCb;
...
function updateDcLocalConfig(uuid, account, datacenter, update, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceUpdateDcLocalConfig(span, tracedCb) {
        cb = tracedCb;
        ...
        self.getDcLocalConfig(uuid, account, datacenter, function (err) {
            if (err && err.statusCode === 404) {
                ...
                var ldapChanges =
                    Object.keys(update).reduce(function (acc, key) {
                    if (key === 'dn' ||
                        key === 'objectclass' ||
                        key === '_owner' ||
                        key === '_parent' ||
                ...
                if (!ldapChanges.length) {
                    ...
                    self.modify(dn, ldapChanges, function (err3) {
                        if (err3) {
                            ...
                            self.getDcLocalConfig(uuid, account, datacenter,
                                cb);
                        }
                    });
                }
...
function deleteDcLocalConfig(uuid, account, datacenter, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceDeleteDcLocalConfig(span, tracedCb) {
        cb = tracedCb;
...
UFDS.prototype.addForeigndc = function addForeigndc(user, authdev, dc, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceAddForeignDc(span, tracedCb) {
        cb = tracedCb;
            ...
            if (err) {
                ...
                self.search(user.dn, opts, function (err, devlist) {
                    ...
                    if (devlist.length !== 0) {
                        ...
                        var insertdevdn = sprintf(AUTHDEV_FMT, authdev,
                            user.uuid);
                        var devobj = {
                            authdev: authdev,
                            objectclass: 'authdev'
                        };
                        self.add(insertdevdn, devobj,
                            function (err2, deventry) {
                            if (err2) {
                                cb(err2);
                                return;
                            }
...
function getMetadata(user, key, account, cb, noCache) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceGetMetadata(span, tracedCb) {
        cb = tracedCb;
...
function addMetadata(user, key, metadata, account, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceAddMetadata(span, tracedCb) {
        cb = tracedCb;
...
function modifyMetadata(user, key, md, account, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceModifyMetadata(span, tracedCb) {
        cb = tracedCb;
...
function deleteMetadata(user, key, account, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceDeleteMetadata(span, tracedCb) {
        cb = tracedCb;
...
UFDS.prototype.listPolicies = function listPolicies(account, cb, noCache) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceListPolicies(span, tracedCb) {
        cb = tracedCb;
        ...
        self.search(dn, opts, function (err, policies) {
            if (err) {
                cb(err);
            } else {
                cb(null, policies);
...
UFDS.prototype.getPolicy = function getPolicy(account, policy, cb, noCache) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceGetPolicy(span, tracedCb) {
        var dn = sprintf(USER_FMT, account);
        var opts = {
            scope: 'one',
            filter: sprintf(
...
function modifyPolicy(account, policy, changes, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceModifyPolicy(span, tracedCb) {
...
UFDS.prototype.deletePolicy = function deletePolicy(account, policy, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceDeletePolicy(span, tracedCb) {
        cb = tracedCb;
        ...
        self.del(sprintf(POLICY_FMT, policy, account), cb);
    });
};
...
UFDS.prototype.listRoles = function listRoles(account, filter, cb, noCache) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceListRoles(span, tracedCb) {
        var opts = {
            scope: 'one',
            filter: filter
        };
...
UFDS.prototype.addRole = function addRole(account, role, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceAddRole(span, tracedCb) {
...
UFDS.prototype.getRole = function getRole(account, role, cb, noCache) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceGetRole(span, tracedCb) {
        var dn = sprintf(USER_FMT, account);
        var opts = {
            scope: 'one',
            filter: sprintf(
                '(&(objectclass=sdcaccountrole)(|(uuid=%s)(name=%s)))', role,
                role)
        };
        ...
        self.search(dn, opts, function (err, roles) {
            if (err) {
                cb(err);
            } else {
                if (!roles.length) {
...
function modifyRole(account, role, changes, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceModifyRole(span, tracedCb) {
...
UFDS.prototype.deleteRole = function deleteRole(account, role, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceDeleteRole(span, tracedCb) {
        cb = tracedCb;
        span.addTags({
            account: account,
            role: role
        ...
        self.del(sprintf(ROLE_FMT, role, account), cb);
    });
};
...
UFDS.prototype.listResources = function listResources(account, cb, noCache) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceListResources(span, tracedCb) {
        var opts = {
            scope: 'one',
...
UFDS.prototype.addResource = function addResource(account, resource, cb) {
    ...
    if (!resource.uuid) {
        resource.uuid = uuidv4();
    }

    cb = once(cb);
    self._traceFunction(cb, function _traceAddResource(span, tracedCb) {
...
function getResource(account, resource, cb, noCache) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceGetResource(span, tracedCb) {
        var dn = sprintf(USER_FMT, account);
        var opts = {
            scope: 'one',
            filter: sprintf(
        ...
        self.search(dn, opts, function (err, resources) {
            if (err) {
                cb(err);
            } else {
                if (!resources.length) {
...
function modifyResource(account, resource, changes, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceModifyResource(span, tracedCb) {
...
UFDS.prototype.deleteResource = function deleteResource(account, resource, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceDeleteResource(span, tracedCb) {
        cb = tracedCb;
        ...
        self.del(sprintf(RESOURCE_FMT, resource, account), cb);
    });
};
...
UFDS.prototype.listDatacenters = function listDatacenters(region, cb, noCache) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceListDatacenters(span, tracedCb) {
...
UFDS.prototype.listResolvers = function listResolvers(region, cb, noCache) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceListResolvers(span, tracedCb) {
        var dn = sprintf(REGION, region);
        var opts = {
            scope: 'sub',
...
/**
 * AccessKeys related methods
 */

/**
 * Adds a new AccessKey to a given user record.
 *
 * The AccessKeyId and AccessKeySecret will be generated.
 *
 * This method will return you the full key as processed by UFDS.
 *
 * @param {Object} user the user record you got from getUser.
 * @param {String} (Optional) account uuid for a customer sub-user
 * @param {Function} callback of the form fn(err, key).
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.addAccessKey = function addAccessKey(user, account, cb) {
    if (typeof (account) === 'function') {
        cb = account;
        account = null;
    }
    ...
    function _addAccKey(init_err, usr) {
        if (init_err) {
            cb(init_err);
            return;
        }
        ...
        try {
            ...
            var accessKeySecret = crypto.randomBytes(32).toString('hex');
            entry.accesskeyid = accessKeyId;
            entry.accesskeysecret = accessKeySecret;
        } catch (e) {
            cb(new InternalError(e.message));
            return;
        }

        var dn = (account) ?
            sprintf(SUBUSER_ACCESS_KEY_FMT,
                entry.accesskeyid, usr.uuid, account) :
            sprintf(USER_ACCESS_KEY_FMT, entry.accesskeyid, usr.uuid);
        ...
        self.add(dn, entry, function (err) {
            ...
            self.getAccessKey(usr, entry.accesskeyid, account, cb);
        });
    }

    if (typeof (user) === 'object') {
        _addAccKey(null, user);
    } else {
        this.getUser(user, account, _addAccKey);
    }
};


/**
 * Retrieves an AccessKey by Id.
 *
 * @param {Object} user the object you got back from getUser.
 * @param {String} AccessKeyId of the AccessKey you want.
 * @param {String} (Optional) account uuid for a customer sub-user.
 * @param {Function} callback of the form fn(err, key).
 * @param {noCache} (Optional) boolean to force a full directory lookup.
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.getAccessKey =
function getAccessKey(user, accesskeyid, account, cb, noCache) {
    if (typeof (account) === 'function') {
        cb = account;
        account = null;
    }
    ...
    var self = this;
    function _accKeySearch(err, usr) {
        if (err) {
            cb(err);
            return;
        }
        var filter = util.format(
            '(&(objectclass=accesskey)(accesskeyid=%s))', accesskeyid);
        var opts = {
            scope: 'one',
            filter: filter
        };
        assert.string(usr.dn, 'user DN must be present and be a string');
        self.search(usr.dn, opts, function (sErr, keys) {
            if (sErr) {
                cb(sErr);
                return;
            }
    ...
    if (typeof (user) === 'object') {
        _accKeySearch(null, user);
    } else {
        self.getUser(user, account, _accKeySearch);
    }
};


/**
 * Loads all the access keys for a given user.
 *
 * @param {Object} user the user you got from getUser.
 * @param {String} (Optional) account uuid for a customer sub-user
 * @param {Function} callback of the form fn(err, keys).
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.listAccessKeys =
function listAccessKeys(user, account, cb, noCache) {
    if (typeof (account) === 'function') {
        cb = account;
        account = null;
    }
    ...
    function _accKeys(err, usr) {
        ...
        var opts = {
            scope: 'one',
            filter: '(objectclass=accesskey)'
        };
        self.search(usr.dn, opts, cb, noCache);
    }

    if (typeof (user) === 'object') {
        _accKeys(null, user);
    } else {
        self.getUser(user, account, _accKeys);
    }
};


/**
 * Deletes an AccessKey under a user.
 *
 * @param {User} the object you got back from getUser.
 * @param {Object} accesskey the object you got from getAccessKey
 *     or {String} the AccessKeyId.
 * @param {String} (Optional) account uuid for a customer sub-user
 * @param {Function} callback of the form fn(err).
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.deleteAccessKey =
function deleteAccessKey(user, accesskey, account, cb) {
    if (typeof (account) === 'function') {
        cb = account;
        account = null;
    }
    ...
    if (typeof (accesskey) !== 'string') {
        assert.object(accesskey, 'accesskey');
    }
    assert.func(cb, 'callback');
    account = workAroundUnderspecification(account, 'account');
    ...
    var self = this;
    function _delAccKey(usr, keyObj) {
        if (!ldap.parseDN(usr.dn).parentOf(keyObj.dn)) {
            cb(new NotAuthorizedError(keyObj.dn + ' not a child of ' + usr.dn));
            return;
        }
        self.del(keyObj.dn, cb);
    }

    function _getAccKey(getErr, usr) {
        if (getErr) {
            cb(getErr);
            return;
        }
        ...
        if (typeof (accesskey) === 'object') {
            _delAccKey(usr, accesskey);
        } else {
            self.getAccessKey(usr, accesskey, account, function (err, keyObj) {
                if (err) {
                    cb(err);
                    return;
                }
                _delAccKey(usr, keyObj);
            });
        }
    }

    if (typeof (user) === 'object') {
        _getAccKey(null, user);
    } else {
        self.getUser(user, account, _getAccKey);
    }
};
...
UFDS.prototype.add = function add(dn, entry, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceAdd(span, tracedCb) {
        cb = tracedCb;
        span.addTags({
            dn: dn
        });
...
UFDS.prototype.del = function del(dn, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceDel(span, tracedCb) {
        cb = tracedCb;
        span.addTags({
            dn: dn
        });
...
UFDS.prototype.modify = function modify(dn, changes, cb) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceModify(span, tracedCb) {
        cb = tracedCb;
        span.addTags({
            dn: dn
        });
...
UFDS.prototype.search = function search(base, options, controls, cb, noCache) {
    ...
    cb = once(cb);
    self._traceFunction(cb, function _traceSearch(span, tracedCb) {
        ...
        if (!noCache && (tmp = (self.cache ? self.cache.get(key) : false))) {
            span.addTags({cacheHit: true});
            cb(null, clone(tmp));
            return;
        } else if (!noCache) {
        ...
        self.client.search(base, options, controls, function (start_err, res) {

            if (self.tracer) {
                self.tracer.cls().bindEmitter(res);
            }
            ...
            res.on('end', function () {
                ...
                span.addTags({entries: entries.length});
...
UFDS.prototype.compare = function compare(dn, attr, val, cb) {
    ...
    self._traceFunction(cb, function _traceModify(span, tracedCb) {
        ...
        self.client.compare(dn, attr, val, cb);
    });
};
...
// XXX Should we switch the parameters?
UFDS.prototype._traceFunction = function _traceFunction(oriCb, handler) {
    var self = this;

    var fields = {};
    var fnName = 'ufds.' + arguments.callee.caller.name;
    var parentSpan;
    var span;

    if (!self.tracer) {
        // NO-OP when tracer was not passed to UFDS constructor
        handler(new DummySpan(), oriCb);
        return;
    }

    parentSpan = self.tracer.getCurrentSpan();
    if (parentSpan) {
        fields.childOf = parentSpan.context();
    }

    span = self.tracer.tracer().startSpan(fnName, fields);
    span.addTags({
        'component': 'node-ufds',
        'span.kind': 'client'
    });
    ...
    self.tracer.cls().run(function _callHandler() {
        self.tracer.setCurrentSpan(span);
        handler(span, function tracedCallback(err) {
            var _self = this;

            span.log({event: 'client-end'});
            ...
            oriCb.apply(_self, arguments);
        });
    });
};

// DummySpan is returned when tracing is disabled but also serves to illustrate
// the functions we expect to be able to call on a span.
function DummySpan() {}
DummySpan.prototype.addTags = function addTags() {};
