...
function extendUser(self, user) {
    ...
    user.groups = function groups() {
        ...
        user.memberof.forEach(function (g) {
            ...
            if (rdns && rdns.length && rdns[0].attrs &&
                rdns[0].attrs.cn && rdns[0].attrs.cn.value) {
                grps.push(rdns[0].attrs.cn.value);
            }
        });
        return (grps);
    };
...
function UFDS(opts) {
    ...
    this.ldapOpts = {
        ...
        url: opts.url,
        idleTimeout: opts.idleTimeout != null ? opts.idleTimeout : 90000,
        queueSize: 10,
        queueTimeout: 200
    };
    if (opts.retry) {
 ...
 * This method will return the full key as processed by UFDS.
 *
 * To preserve compatibiliy with any existing clients that were using this
 * method before adding the `attrs` argument was added and to align with
 * conventions pre-existing in this class, this method may be called with
 * any of the following signatures:
 *
 *   addAccessKey(user, cb);
 *   addAccessKey(user, attrs, cb);
 *   addAccessKey(user, account, cb);
 *   addAccessKey(user, account, attrs, cb);
 *
 * @param {Object|String} the user record you got from getUser.
 * @param {String} (Optional) account uuid for a customer sub-user
 * @param {String} (Optional) attrs for the access key
 * @param {Function} callback of the form fn(err, key).
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.addAccessKey = function addAccessKey(user, account, attrs, cb) {
    ...
    if (typeof (account) === 'function') {
        ...
        cb = attrs;
        // addAccessKey(user, attrs, cb)
        if (argc === 3 && account && typeof (account) !== 'string') {
            attrs = account;
            account = null;
        // addAccessKey(user, account, cb)
        } else {
    ...
    vasync.waterfall([
        ...
        function getUserObject(next) {
            if (typeof (user) === 'object') {
                next(null, user);
            } else {
                self.getUser(user, account, next);
            }
        },

        function generateSecretKey(user, next) {
            const prefix = accesskey.DEFAULT_PREFIX;
            const bytes = accesskey.DEFAULT_BYTE_LENGTH;
            accesskey.generate(prefix, bytes, function (err, secret) {
                var errMsg = 'Failed to generate accesskeysecret';
                if (err) {
                    self.log.error(errMsg, {err: err});
                    next(new InternalError(err.message));
                    return;
                }
                if (!accesskey.validate(prefix, bytes, secret)) {
                    errMsg = 'Generated accesskeysecret failed validation';
                    self.log.error(errMsg, {secret: secret});
                    next(new InternalError(errMsg));
                    return;
                }
                next(null, {user: user, secret: secret});
                return;
            });

        },

        function generateId(context, next) {
            crypto.randomBytes(16, function (err, id) {
                if (err) {
                    var errMsg = 'Failed to generate accesskeyid';
                    self.log.error(errMsg, {err: err});
                    next(new InternalError(errMsg));
                } else {
                    context.id = id.toString('hex');
                    next(null, context);
                }
            });
        },

        function addEntry(context, next) {
            var entry = clone(attrs);
            entry.objectclass = 'accesskey';
            entry.updated = entry.created = Date.now();
            ...
            var dn = (account) ?
                sprintf(SUBUSER_ACCESS_KEY_FMT,
                    entry.accesskeyid, userUuid, account) :
                sprintf(USER_ACCESS_KEY_FMT, entry.accesskeyid, userUuid);
        ...
        function returnAccessKey(context, next) {
            self.getAccessKey(context.user, context.id, account, next);
        }
...
function listAccessKeys(user, account, cb, noCache) {
    ...
    vasync.waterfall([
        function getFullUserObject(next) {
            if (typeof (user) === 'object') {
                next(null, user);
            } else {
                self.getUser(user, account, next);
        ...
        function searchForKeys(user, next) {
            var opts = {
                scope: 'one',
                filter: '(objectclass=accesskey)'
            };
            self.search(user.dn, opts, function (err, entries) {
                if (err) {
                    next(err);
                } else {
...
/**
 * Convenience function to load all the Active access keys for a given user.
 *
 * @param {Object} user the user you got from getUser.
 * @param {String} (Optional) account uuid for a customer sub-user
 * @param {Function} callback of the form fn(err, keys).
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.listActiveAccessKeys =
function listActiveAccessKeys(user, account, cb, noCache) {
    if (typeof (account) === 'function') {
        cb = account;
        account = null;
    }
    ...
    vasync.waterfall([
        ...
        function searchForKeys(user, next) {
            var opts = {
                scope: 'one',
                filter: '(&(objectclass=accesskey)(status=Active))'
            };
            self.search(user.dn, opts, next, noCache);
        }
    ], cb);
};
...
/**
 * Updates an AccessKey under a user.
 *
 ...
 *
 *   updateAccessKey(user, accesskey, cb);
 *   updateAccessKey(user, account, accesskey, cb);
 *
 * @param {Object} user record from getUser.
 * @param {String} (Optional) account uuid for a customer sub-user.
 * @param {Object} accesskey object must contain at least an accesskeyid
 *                 property and the attributes you wish to update.
 *                 You may pass in a null value to delete an attr.
 * @param {Function} callback of the form fn(err, key).
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.updateAccessKey =
function updateAccessKey(user, account, accesskey, cb) {

    // updateAccessKey(user, accesskey, cb)
    if (typeof (account) === 'object' && typeof accesskey === 'function') {
        cb = accesskey;
        accesskey = account;
        account = null;
    }
    ...
    vasync.waterfall([
        ...
        function getFullAccessKey(user, next) {
            if (typeof (accesskey) === 'object' &&
                typeof (accesskey.dn) === 'string') {
                next(null, {
                    user: user,
                    accesskey: accesskey
                });
            ...
            if (typeof (accesskey.accesskeyid) !== 'string') {
                var errMsg = 'accesskeyid must be a string';
                next(InvalidArgumentError(errMsg));
                return;
            }

            self.getAccessKey(user, accesskey.accesskeyid, account,
                function (err, keyObj) {
                if (err) {
                    next(err);
                } else {
        ...
        function UpdateAccessKey(context, next) {
            assert.string(context.user.dn, 'userDn');
            assert.string(context.accesskey.dn, 'accesskeyDn');
            var userDn = context.user.dn;
            var accesskeyDn = context.accesskey.dn;

            if (!ldap.parseDN(userDn).parentOf(accesskeyDn)) {
                var errMsg = accesskeyDn + ' not a child of ' + userDn;
                next(new NotAuthorizedError(errMsg));
                return;
            }
            ...
            Object.keys(accesskey).forEach(function _keys(key) {
                ...
                if (ignoreAttrs.indexOf(key) !== -1 ||
                    context.accesskey[key] === accesskey[key]) {
                    return;
                }
                ...
                if (context.accesskey.hasOwnProperty(key)) {
                    change.type = 'replace';
                    change.modification[key] = accesskey[key];
                    changes.push(change);
                    return;
                }
            ...
            self.modify(accesskeyDn, changes, function _modify(err) {
                if (err) {
                    next(err);
                } else {
                    next(null, context);
        ...
        function returnAccessKey(context, next) {
            var accesskeyid = context.accesskey.accesskeyid;
            self.getAccessKey(context.user, accesskeyid, account,
                function _getAccessKey(err, key) {
                if (err) {
                    next(err);
                } else {
                    next(null, key);
