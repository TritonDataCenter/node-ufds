...
function traceFunction(cb, traceTags) {
    var fnName = 'ufds.' + arguments.callee.caller.name;
    var spanStart = (new Date()).getTime();

    return function _tracedCallback(err) {
        var self = this;
        ...
        tritonTracer.localSpan(fnName, {}, function _wrappedAuth(err, span) {
            ...
            span.log({event: 'local-begin', timestamp: spanStart});
    ...
    function _compare(user) {
        self.compare(user.dn, 'userpassword', pwd, function (err, ok) {
            if (err) {
                ...
                traceTags.invalidCredentials = true;
