...
function getAccessKey(user, id, account, cb, noCache) {
    if (typeof (account) === 'function') {
        cb = account;
        account = null;
    }
    ...
    function _accKeySearch(err, usr) {
        ...
        var filter = util.format(
            '(&(objectclass=accesskey)(accesskeyid=%s))', id);
        var opts = {
            scope: 'one',
            filter: filter
        };
        self.search(usr.dn, opts, function (sErr, keys) {
            ...
            cb(new ResourceNotFoundError(id + ' does not exist'));
        }, noCache);
    }
 ...
 *
 * @param {User} the object you got back from getUser.
 * @param {Object} key the object you got from getAccessKey or the AccessKeyId
 * @param {String} (Optional) account uuid for a customer sub-user
 * @param {Function} callback of the form fn(err).
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.deleteAccessKey =
function deleteAccessKey(user, key, account, cb) {
    if (typeof (account) === 'function') {
        cb = account;
        account = null;
    }
    ...
    function _getAccKey(usr) {
        if (typeof (key) === 'object') {
            _delAccKey(usr, key);
        } else {
            self.getAccessKey(usr, key, account, function (err, keyObj) {
                if (err) {
                    cb(err);
                    return;
                }
    ...
    if (typeof (user) === 'object') {
        ...
        this.getUser(user, account, function (err, userObj) {
            if (err) {
                cb(err);
                return;
            }
